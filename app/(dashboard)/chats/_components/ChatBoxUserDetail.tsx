...
export default function ChatBoxUserDetail({ selectedChat, receiver }: { selectedChat: Chat, receiver: any }) {
        ...
        <div className="flex items-center space-x-2 rtl:space-x-reverse cursor-pointer">
            ...
            <div className="relative flex-none">
                <Image
                    width={40}
                    height={40}
                    src={`${selectedChat?.is_group_chat ? `${BASE_URL}/image/${selectedChat?.chat_picture || '/assets/images/profile.png'}` : `${BASE_URL}/image/${receiver?.profilePic || '/assets/images/profile.png'}`}`}
            ...
            <div className="mx-3">
                <p className="font-semibold">{selectedChat.is_group_chat ? `${selectedChat?.chat_name}` : `${receiver?.firstName} ${receiver?.lastName}`}</p>
